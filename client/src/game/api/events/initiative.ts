...
socket.on("Initiative.Value.Set", (data: { shape: string; value: number }) =>
    initiativeStore.setInitiative(UuidToIdMap.get(data.shape)!, data.value, false),
);
socket.on("Initiative.Remove", (data: string) => initiativeStore.removeInitiative(UuidToIdMap.get(data)!, false));
...
socket.on("Initiative.Effect.New", (data: { actor: string; effect: InitiativeEffect }) => {
    initiativeStore.createEffect(UuidToIdMap.get(data.actor)!, data.effect, false);
});
socket.on("Initiative.Effect.Rename", (data: { shape: string; index: number; name: string }) => {
    initiativeStore.setEffectName(UuidToIdMap.get(data.shape)!, data.index, data.name, false);
});
socket.on("Initiative.Effect.Turns", (data: { shape: string; index: number; turns: string }) => {
    initiativeStore.setEffectTurns(UuidToIdMap.get(data.shape)!, data.index, data.turns, false);
});
socket.on("Initiative.Effect.Remove", (data: { shape: string; index: number }) =>
    initiativeStore.removeEffect(UuidToIdMap.get(data.shape)!, data.index, false),
);
socket.on("Initiative.Option.Set", (data: { shape: string; option: "isVisible" | "isGroup"; value: boolean }) =>
    initiativeStore.setOption(UuidToIdMap.get(data.shape)!, data.option, data.value),
);
