...
export class GridLayer extends Layer implements IGridLayer {
    ...
    show(): void {
        // if (locationSettingsState.raw.useGrid.value && this.floor === floorState.readonly.currentFloor!.id)
        //     this.canvas.style.removeProperty("display");
    }

    draw(_doClear?: boolean): void {
        if (!this.valid) {
            if (true || locationSettingsState.raw.useGrid.value) {
                // const activeFowFloor = floorState.readonly.currentFloor!.id;
