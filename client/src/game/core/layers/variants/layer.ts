...
export class Layer implements ILayer {
    ...
    constructor(
        ...
        this.ctx = canvasInfo.canvas.getContext("2d")!;
    }
    ...
    get isActiveLayer(): boolean {
        return floorState.readonly.currentLayer === this;
    }
    ...
    draw(doClear = true): void {
        if (!this.valid) {
            ...
            if (this.name !== LayerName.Lighting && this.selectable) {
                if (floorState.readonly.layerIndex < this.index) ctx.globalAlpha = 0.3;
            ...
            if (playerSettingsState.raw.showTokenDirections.value) {
                if (this.floor === floorState.readonly.currentFloor?.id && this.name === LayerName.Draw) {
