...
export function createShapeFromDict(shape: ApiShape): IShape | undefined {
    ...
    try {
        ...
        if (shape.type_ === "rect") {
            ...
            sh = new Asset(refPoint, asset.width, asset.height, { uuid, loaded: false });

            let url = baseAdjust(asset.src);
            if (asset.src.startsWith("http")) url = baseAdjust(new URL(asset.src).pathname);
            (sh as Asset).setImage(url, false).catch((e) => console.error(e));
