...
export class Asset extends BaseRect implements IAsset {
    ...
    src = "";
    #loaded: boolean;
    private img: ImageBitmap | undefined;
    ...
    draw(ctx: CanvasContext, customScale?: { center: GlobalPoint; width: number; height: number }): void {
        ...
        if (!this.#loaded || this.img === undefined) {
    ...
    async setImage(url: string, sync: boolean): Promise<void> {
        this.#loaded = false;
        this.src = url;
        ...
        if (sync) {
            ...
            if (uuid) sendAssetRectImageChange({ uuid, src: url });
        }
    }
}
