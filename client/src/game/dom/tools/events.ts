...
import { getActiveTool, getFeatures, toolMap } from "./tools";
...
export async function mouseMove(event: MouseEvent): Promise<void> {
    ...
    //     if (floorSystem.hasLayer(floorState.readonly.currentFloor!, LayerName.Draw)) {
    //         const shape = getShape(uuid);
    //         if (shape && shape.floorId === floorState.readonly.currentFloor!.id && shape.contains(eventPoint)) {
...
export async function touchMove(event: TouchEvent): Promise<void> {
    ...
    //         if (
    //             shape &&
    //             shape.floorId === floorState.readonly.currentFloor!.id &&
