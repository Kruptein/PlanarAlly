...
// import { gameSystem } from "../../../core/systems/game";
...
import { activeTool, activeToolMode } from "../../tools/state";
import { activeModeTools, dmTools, toggleActiveMode, toolMap } from "../../tools/tools";
...
const hasGameboard = false; // coreStore.state.boardId !== undefined;
...
const visibleTools = computed(() => {
    {
        ...
        for (const [toolName] of activeModeTools.value) {
            if (dmTools.includes(toolName) && !uiGameState.reactive.isDm) continue;
...
watch(
    // () => playerSettingsState.reactive.useToolIcons.value
    [activeTool, activeToolMode, visibleTools],
    () => updateDetails(),
    { flush: "post" },
);
