...
async def cleanup_assets(entries: list[ApiAssetEntry]):
    for entry in entries:
        if entry.asset and entry.asset.id:
            asset = Asset.get_by_id(entry.asset.id)
...
async def handle_regular_file(
    upload_data: ApiAssetUpload,
    data: bytes,
    kind: Literal["regular", "ddraft"],
    name: str,
    extension: str | None,
    sid: str,
):
...
async def assetmgmt_upload(sid: str, raw_data: Any):
    ...
    if upload_data.name.endswith(".dd2vtt"):
        *name, extension = upload_data.name.split(".")
        ...
        if len(split_name) > 1:
            cleaned_name = ".".join(split_name[:-1])
