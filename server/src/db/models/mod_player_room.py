...
class ModPlayerRoom(BaseDbModel):
    ...
    mod = cast(
        Mod, ForeignKeyField(Mod, backref="mods_player_room", on_delete="CASCADE")
    )
    player_room = cast(
        PlayerRoom,
    ...
    def __repr__(self):
        return f"<ModPR {self.mod.tag}-{self.mod.version} - {self.player_room.room.name} - {self.player_room.player.name}>"
