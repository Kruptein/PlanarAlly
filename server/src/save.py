...
def upgrade(
    ...
    if version == 94:
        ...
        with db.atomic():
            ...
            db.execute_sql("DROP TABLE asset")
            db.execute_sql(
                'CREATE TABLE IF NOT EXISTS "asset" ("id" INTEGER NOT NULL PRIMARY KEY, "file_hash" TEXT NOT NULL, "kind" TEXT NOT NULL, "extension" TEXT)'
            )
            ...
            for shape_id, src, asset_id, width, height in shape_data:
                ...
                # Spawn shapes will become FA icons instead of AssetRects
                if src.endswith("spawn.png"):
                    spawn_shapes.append(shape_id)
                    continue
                elif src_hash not in file_map:
                    ...
                    query = db.execute_sql(
                        "INSERT INTO asset (file_hash, kind) VALUES (?,?) RETURNING id",
                        (src_hash, "regular"),
                    )
                    asset_id = query.fetchone()[0]
            ...
            # STEP 8: MIGRATE SPAWN ICONS

            db.execute_sql(
                'CREATE TABLE IF NOT EXISTS "font_awesome" ("shape_id" TEXT NOT NULL PRIMARY KEY, "width" REAL NOT NULL, "height" REAL NOT NULL, "icon_prefix" TEXT NOT NULL, "icon_name" TEXT NOT NULL, FOREIGN KEY ("shape_id") REFERENCES "shape" ("uuid") ON DELETE CASCADE)'
            )

            for shape_id in spawn_shapes:
                db.execute_sql(
                    "INSERT INTO font_awesome (shape_id, width, height, icon_prefix, icon_name) VALUES (?, ?, ?, ?, ?)",
                    (shape_id, 50, 50, "fas", "arrows-to-circle"),
                )
                db.execute_sql(
                    "UPDATE shape SET (type_, fill_colour, stroke_colour, stroke_width) = ('fontawesome', '#000', '#fff', 10) WHERE uuid = ?",
                    (shape_id,),
                )
