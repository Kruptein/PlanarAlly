...
def upgrade(db: SqliteExtDatabase, version: int):
    ...
    if version == 64:
        ...
        # Add DataBlock related things
        with db.atomic():
            db.execute_sql(
                'CREATE TABLE IF NOT EXISTS "data_block" ("id" INTEGER NOT NULL PRIMARY KEY, "source" TEXT NOT NULL, "name" TEXT NOT NULL, "data" TEXT NOT NULL)'
            )
            db.execute_sql(
                'CREATE UNIQUE INDEX "data_block_source_name" ON "data_block" ("source", "name");'
            )
            db.execute_sql(
            ...
            db.execute_sql(
                'CREATE TABLE IF NOT EXISTS "character_data_block" ("id" INTEGER NOT NULL PRIMARY KEY, "character_id" INTEGER NOT NULL, "data_block_id" INTEGER NOT NULL, FOREIGN KEY ("character_id") REFERENCES "character" ("id") ON DELETE CASCADE, FOREIGN KEY ("data_block_id") REFERENCES "data_block" ("id") ON DELETE CASCADE)'
            )
