...
def get_data_block(model: ApiDataBlock, pr: PlayerRoom) -> DataBlock | None:
    if model.category == "room":
        return RoomDataBlock.get_or_none(
            source=model.source, name=model.name, room=pr.room
        )
        ...
        return UserDataBlock.get_or_none(
            source=model.source, name=model.name, user=pr.player
        )
